...
*/
	for (;;)
	{
		...
		case -1:				/* Have not read line with keyword */
			...
			warning_msg(error_string);
			while ((j =
					check_line("No keyword", FALSE, TRUE, TRUE,
							   TRUE)) != KEYWORD && j != EOF)
			{
				warning_msg(error_string);
			}
...
# else
		...
		case 49:
			...
			{
				error_msg
					("DATABASE must be the first keyword in the input file.",
					 CONTINUE);
				input_error++;
			}
			else
 ...
 */
	...
	for (;;)
	{
		...
		case 4:				/* gammas */
			exchange[n].pitzer_exchange_gammas =
		...
		case OPTION_DEFAULT:
			...
			get_token(&ptr, token1,
					  &exchange[n].comps[count_comps].formula_z, &l);
 ...
 */
		...
		{
			mix[n].comps =
				(struct mix_comp *) PHRQ_malloc((size_t)
